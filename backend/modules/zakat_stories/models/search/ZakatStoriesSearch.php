...
namespace backend\modules\zakat_stories\models\search;

use backend\modules\zakat_stories\models\ZakatStories;
use backend\modules\zakat_stories\models\ZakatStoriesLang;
use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;

/**
 * ZakatStoriesSearch represents the model behind the search form about `common\models\Blogs`.
 */
class ZakatStoriesSearch extends ZakatStories
{
    public $year=null;
    ...
    {
        $query = ZakatStories::find()->joinWith('translations');
       ...
       if (!$this->validate()) {
        ...
        $query->andFilterWhere( 
            ['not',
                ['and',  
                    [
                        'and',
                        [ZakatStories::tableName().'.changed'=> 1],
                        ['!=', ZakatStories::tableName().'.revision', -1],
                    ], 
                    [
                        'and',
                        [ZakatStories::tableName().'.changed'=> 1],
                        ['!=', ZakatStories::tableName().'.revision', 0],
                    ]
                ]
            ]
        );
        ...
        $query->andFilterWhere([
            ZakatStories::tableName().'.id' => $this->id,
            'created_by' => $this->created_by,
            'updated_by' => $this->updated_by,
            ZakatStories::tableName().'.status' => $this->status,
        ]);
        ...
        $query->distinct(true);
        $query->groupBy(ZakatStories::tableName().'.id');
    ...
    public function basicSearch($params)
    {
        ...
        $query->andFilterWhere([
            ZakatStories::tableName().'.status' => 1
        ]);
