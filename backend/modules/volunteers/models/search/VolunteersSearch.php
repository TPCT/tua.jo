...
namespace backend\modules\volunteers\models\search;

use backend\modules\volunteers\models\Volunteers;
use backend\modules\volunteers\models\VolunteersLang;
use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;

/**
 * NewsSearch represents the model behind the search form about `common\models\Blogs`.
 */
class VolunteersSearch extends Volunteers
{
    ...
    {
        $query = Volunteers::find()->joinWith('translations');
       ...
       if (!$this->validate()) {
        ...
        $query->andFilterWhere( 
            ['not',
                ['and',  
                    [
                        'and',
                        [Volunteers::tableName().'.changed'=> 1],
                        ['!=', Volunteers::tableName().'.revision', -1],
                    ], 
                    [
                        'and',
                        [Volunteers::tableName().'.changed'=> 1],
                        ['!=', Volunteers::tableName().'.revision', 0],
                    ]
                ]
            ]
        );
        ...
        $query->andFilterWhere([
            Volunteers::tableName().'.id' => $this->id,
            'created_by' => $this->created_by,
            'updated_by' => $this->updated_by,
            Volunteers::tableName().'.status' => $this->status,
        ]);
        ...
        $query->distinct(true);
        $query->groupBy(Volunteers::tableName().'.id');
