...
class NewsSearch extends News
{
    ...
    {
        $query = News::find()->joinWith('translations');
       ...
       if (!$this->validate()) {
        ...
        $query->andFilterWhere( 
            ['not',
                ['and',  
                    [
                        'and',
                        [News::tableName().'.changed'=> 1],
                        ['!=', News::tableName().'.revision', -1],
                    ], 
                    [
                        'and',
                        [News::tableName().'.changed'=> 1],
                        ['!=', News::tableName().'.revision', 0],
                    ]
                ]
            ]
        );
        ...
        $query->andFilterWhere([
            ...
            News::tableName().'.status' => $this->status,
        ]);
        ...
        if($this->year)
        {
            $query->andFilterWhere(["(date_format(FROM_UNIXTIME(news.`published_at` ), '%Y' ))" => $this->year]);
        }

        $query->andFilterWhere(['like', 'title', $this->title]);

        $query->distinct(true);
        $query->groupBy(News::tableName().'.id');
