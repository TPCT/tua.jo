...
namespace backend\modules\offered_tenders\models\search;

use backend\modules\offered_tenders\models\OfferedTenders;
use backend\modules\offered_tenders\models\OfferedTendersLang;
use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;

/**
 * OfferedTendersSearch represents the model behind the search form about `common\models\Blogs`.
 */
class OfferedTendersSearch extends OfferedTenders
{
    public $year=null;
    ...
    {
        $query = OfferedTenders::find()->joinWith('translations');
       ...
       if (!$this->validate()) {
        ...
        $query->andFilterWhere( 
            ['not',
                ['and',  
                    [
                        'and',
                        [OfferedTenders::tableName().'.changed'=> 1],
                        ['!=', OfferedTenders::tableName().'.revision', -1],
                    ], 
                    [
                        'and',
                        [OfferedTenders::tableName().'.changed'=> 1],
                        ['!=', OfferedTenders::tableName().'.revision', 0],
                    ]
                ]
            ]
        );
        ...
        $query->andFilterWhere([
            OfferedTenders::tableName().'.id' => $this->id,
            'created_by' => $this->created_by,
            'updated_by' => $this->updated_by,
            OfferedTenders::tableName().'.status' => $this->status,
        ]);
        ...
        $query->distinct(true);
        $query->groupBy(OfferedTenders::tableName().'.id');
    ...
    public function basicSearch($params)
    {
        ...
        $query->andFilterWhere([
            OfferedTenders::tableName().'.status' => 1
        ]);
