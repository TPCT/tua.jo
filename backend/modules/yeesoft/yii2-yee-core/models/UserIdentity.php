...
abstract class UserIdentity extends ActiveRecord implements IdentityInterface
{
     ...
     * @inheritdoc
     */
    public static function findInActiveIdentityByAccessToken($token, $type = null)
    {
        return static::findOne(['auth_key' => $token, 'status' => User::STATUS_INACTIVE]);
    }
    ...
    public static function findEmailNotConfirmedByConfirmationToken($token)
    {
        $expire = Yii::$app->yee->confirmationTokenExpire;
    ...
    public static function findEmailNotConfirmedByConfirmationTokenWithoutValidateToken($token)
    {
        return static::findOne([
                    'confirmation_token' => $token,
                    'status' => User::STATUS_ACTIVE,
    ...
    {
        $this->confirmation_token = rand(1000, 9999) . '_' . Yii::$app->security->generateRandomString() . '_' . time();
    }
