...
<?php if (true || $mode !== 'modal'): ?>
    <div class="clearfix">

            <a target="_blank" href="<?= $model->getThumbUrl('original') ?>"><?= Html::img($model->getThumbUrl('original')) ?></a>

        <ul class="detail">
            <li><b><?= Yii::t('yee', 'Author') ?>
                    :</b> <?= ($model->created_by) ? (($model->creator) ? $model->creator->username : 'DELETED') : 'GUEST' ?>
            </li>
            ...
            <li><b><?= Yii::t('yee/media', 'File Path') ?>:</b> <?= $model->url ?></li>
        </ul>
    </div>
<?php endif; ?>
...
<?php if ($mode !== 'modal'): ?>
    ...
    <?php if (User::hasPermission('editMedia')): ?>
        <?= $form->field($model, 'album_id')->dropDownList(ArrayHelper::merge([NULL => Yii::t('yee', 'Not Selected')], Album::getAlbums(true, true)), ['class'=>'form-select'] ) ?>
        <?= $form->field($model, 'title')->textInput(['class' => 'form-control input-sm']); ?>
    <?php else: ?>
        <?= $form->field($model, 'album_id')->dropDownList(ArrayHelper::merge([NULL => Yii::t('yee', 'Not Selected')], Album::getAlbums(true, true)), ['readonly' => 'readonly', 'class'=>'form-select']) ?>
        <?= $form->field($model, 'title')->textInput(['class' => 'form-control input-sm', 'readonly' => 'readonly']); ?>
    <?php endif; ?>

<?php endif; ?>

<?php if (true || $model->isImage()) : ?>
    <?php if (User::hasPermission('editMedia')): ?>
...
<?php if (!$model->isImage()) : ?>
    <?php $form->field($model, 'published_at')->textInput()->hint('Format: YYYY-MM-DD'); ?>
...
<script>
    $(function() {
        $("#media-published_at").inputmask("yyyy-mm-dd", {"placeholder": "yyyy-mm-dd"});
        // $("#media-published_at").inputmask();
    });
</script>
