...
namespace backend\modules\e_card\models\search;
...
/**
 * ECardSearch represents the model behind the search form about `common\models\ECard`.
 */
class ECardSearch extends ECard
{
    ...
    {
        $query = ECard::find()->joinWith('translations');
       ...
       if (!$this->validate()) {
        ...
        $query->andFilterWhere( 
            ['not',
                ['and',  
                    [
                        'and',
                        [ECard::tableName().'.changed'=> 1],
                        ['!=', ECard::tableName().'.revision', -1],
                    ], 
                    [
                        'and',
                        [ECard::tableName().'.changed'=> 1],
                        ['!=', ECard::tableName().'.revision', 0],
                    ]
                ]
            ]
        );
        ...
        $query->andFilterWhere([
            ECard::tableName().'.id' => $this->id,
            'created_by' => $this->created_by,
            'updated_by' => $this->updated_by,
            ECard::tableName().'.status' => $this->status,
        ]);
        ...
        $query->distinct(true);
        $query->groupBy(ECard::tableName().'.id');
