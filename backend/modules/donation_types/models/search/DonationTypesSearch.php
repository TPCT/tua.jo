...
namespace backend\modules\donation_types\models\search;

use backend\modules\donation_types\models\DonationTypes;
use backend\modules\donation_types\models\DonationTypesLang;
use Yii;
use yii\base\Model;
 ...
 */
class DonationTypesSearch extends DonationTypes
{
    public $year=null;
    ...
    {
        $query = DonationTypes::find()->joinWith('translations');
       ...
       if (!$this->validate()) {
        ...
        $query->andFilterWhere( 
            ['not',
                ['and',  
                    [
                        'and',
                        [DonationTypes::tableName().'.changed'=> 1],
                        ['!=', DonationTypes::tableName().'.revision', -1],
                    ], 
                    [
                        'and',
                        [DonationTypes::tableName().'.changed'=> 1],
                        ['!=', DonationTypes::tableName().'.revision', 0],
                    ]
                ]
            ]
        );
        ...
        $query->andFilterWhere([
            DonationTypes::tableName().'.id' => $this->id,
            'created_by' => $this->created_by,
            'updated_by' => $this->updated_by,
            DonationTypes::tableName().'.status' => $this->status,
        ]);
        ...
        $query->distinct(true);
        $query->groupBy(DonationTypes::tableName().'.id');
    ...
    public function basicSearch($params)
    {
        ...
        $query->andFilterWhere([
            DonationTypes::tableName().'.status' => 1
        ]);
