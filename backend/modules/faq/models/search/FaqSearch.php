...
namespace backend\modules\faq\models\search;
...
class FaqSearch extends Faq
{
    ...
    {
        $query = Faq::find()->joinWith('translations');
       ...
       if (!$this->validate()) {
        ...
        $query->andFilterWhere( 
            ['not',
                ['and',  
                    [
                        'and',
                        [Faq::tableName().'.changed'=> 1],
                        ['!=', Faq::tableName().'.revision', -1],
                    ], 
                    [
                        'and',
                        [Faq::tableName().'.changed'=> 1],
                        ['!=', Faq::tableName().'.revision', 0],
                    ]
                ]
            ]
        );
        ...
        $query->andFilterWhere([
            Faq::tableName().'.id' => $this->id,
            'created_by' => $this->created_by,
            'updated_by' => $this->updated_by,
            Faq::tableName().'.status' => $this->status,
        ]);
        ...
        $query->distinct(true);
        $query->groupBy(Faq::tableName().'.id');
