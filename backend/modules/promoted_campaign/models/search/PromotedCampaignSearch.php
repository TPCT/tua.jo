...
namespace backend\modules\promoted_campaign\models\search;
...
/**
 * PromotedCampaignSearch represents the model behind the search form about `common\models\PromotedCampaign`.
 */
class PromotedCampaignSearch extends PromotedCampaign
{
    ...
    {
        $query = PromotedCampaign::find()->joinWith('translations');
       ...
       if (!$this->validate()) {
        ...
        $query->andFilterWhere( 
            ['not',
                ['and',  
                    [
                        'and',
                        [PromotedCampaign::tableName().'.changed'=> 1],
                        ['!=', PromotedCampaign::tableName().'.revision', -1],
                    ], 
                    [
                        'and',
                        [PromotedCampaign::tableName().'.changed'=> 1],
                        ['!=', PromotedCampaign::tableName().'.revision', 0],
                    ]
                ]
            ]
        );
        ...
        $query->andFilterWhere([
            PromotedCampaign::tableName().'.id' => $this->id,
            'created_by' => $this->created_by,
            'updated_by' => $this->updated_by,
            PromotedCampaign::tableName().'.status' => $this->status,
        ]);
        ...
        $query->distinct(true);
        $query->groupBy(PromotedCampaign::tableName().'.id');
    ...
    public function basicSearch($params)
    {
        ...
        $query->andFilterWhere([
            PromotedCampaign::tableName().'.status' => 1
        ]);
