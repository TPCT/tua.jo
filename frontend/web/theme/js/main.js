...
function addToCart(form, redirect=false, show=false){
  form = $(form);
...
$(document).on('click', function () {
  let donationBox = $('.floationg-quick-donation');
  let toolbar = $('#pojo-a11y-toolbar.pojo-a11y-toolbar-left');

  let isDonationOpen = donationBox.hasClass('open');
  let isToolbarOpen = toolbar.hasClass('pojo-a11y-toolbar-open');

  if (isDonationOpen && isToolbarOpen) {
    toolbar.removeClass('pojo-a11y-toolbar-open');
  } else if (isDonationOpen) {
    toolbar.removeClass('pojo-a11y-toolbar-open');
  } else if (isToolbarOpen) {
    donationBox.removeClass('open');
  }
});

$(document).ready(function() {
    setTimeout(function() {
      $('#pojo-a11y-toolbar.pojo-a11y-toolbar-left').css('opacity', '1');
    }, 2000);
  });